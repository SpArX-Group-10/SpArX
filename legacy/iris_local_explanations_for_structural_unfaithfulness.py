...
# creating the results directory for saving the deep models and the
...
# class_names = ['No', 'Yes'] #in case of compas dataset, the output could be 'No' and 'Yes'
# hidden layers
HIDDEN_LAYERS = [50, 50]
...
pruning_ratio = 0.5  # the number should be in range [0, 1].
...
# One-hot
# Make data as one-hot, also make X as one-hot #for iris there is no need to change the dataset to one-hot
...
#     train_test_split(X, y, data, X_onehot, y_onehot, data_onehot, test_size=.25, random_state=2020, shuffle=True)
# for iris
...
# 'net_iris_local.h5')
model_path = os.path.join(RESULT_PATH, 'net_iris_local.h5')
forge_gen = False
# load model weights if previously trained and saved. If not, train the model.
if not os.path.exists(model_path) or forge_gen:
    #     history = utility_functions.net_train(model, model_path, X_onehot_train, y_onehot_train, X_onehot_test,
    #                                           y_onehot_test)
    history = utility_functions.net_train(
        model,
        model_path,
        X_train,
        y_onehot_train,
...
#     score = model.evaluate(X_onehot_test, y_onehot_test)
    ...
    plt.grid(alpha=.2)
    plt.title(
        f'batch_size = {utility_functions.BATCH_SIZE}, epochs = {utility_functions.EPOCHS}')
    plt.draw()
else:
    print('Model loaded.')
...
# this function assigns the zero activations for each node to a separate
# cluster of zero activations.


def clustering_the_zero_activations_out_to_a_new_cluster_for_an_example(
        activations, hidden_layers, index, clustering_lables_global):
        ...
        for idx, activation in enumerate(activations[layer][index]):
            if activation == 0:
                ...
                labels_after_adding_cluster_of_zeros.append(
                    clustering_lables_global[layer][idx] + 1)
                # label_index += 1
        clustering_labels_after_adding_zero_cluster.append(
            np.array(labels_after_adding_cluster_of_zeros))
        ...
        for label in range(
            ...
            if len(np.vstack(
                ...
                h_star = utility_functions.compute_activations_for_each_layer(
                    shrunken_model, input.reshape((1, -1)))[index][0, label]
                all_hidden_activations = activations[index][example_index,
                ...
                outgoing_weights[index +
                                 1].append(np.dot(activations_divided_by_h_star, model.layers[index +
                                                                                              1].get_weights()[0][clustering_labels[index] == label]).reshape((1, -
                                                                                                                                                               1)))
            else:
    ...
    for index, hidden_layer in enumerate(HIDDEN_LAYERS):
        ...
        for label in range(
            ...
            if len(np.vstack(
                ...
                #
                activations_divided_by_h_star = np.sum(
                    np.multiply(
                        all_hidden_activations.T,
                        new_example_weights /
                        h_star).T,
                    axis=0)
                ...
                outgoing_weights[index +
                                 1].append(np.dot(activations_divided_by_h_star, model.layers[index +
                                                                                              1].get_weights()[0][clustering_labels[index] == label]))
            else:
                weights[index].append(np.zeros(input_size) if index == 0 else np.zeros(
                    int((preserve_percentage / 100) * HIDDEN_LAYERS[index - 1])))
                biases[index].append(0.0)
...
for Shrinkage_percentage in range(20, 90, 20):
    ...
    # Shrink the network using the Kmeans clustering below. #Here we shrink
    # the hidden nodes from 100 nodes to 3 nodes
    ...
    # this means that instead of sum for the output we use weighted sum
    # weighted based on activation values
    ...
    for j in range(0, number_of_tests):
        ...
        for example_index in range(
                0, number_of_examples):  # len(X_onehot_test)):

            truncated_model_dimensions = [int(
                (preserve_percentage / 100) * hidden_layer) for hidden_layer in HIDDEN_LAYERS]

            # cunstruct the structure of the truncated model using the same
            # function for building a FFNN model.
            ...
            distance_examples_to_target = sklearn.metrics.pairwise_distances(
                X_test.values,
                X_test.values[example_index].reshape(1, -1),
                metric='euclidean'
            ).ravel()
            ...
            clustering_labels = clustering_nodes_weighted(
                preserve_percentage,
                HIDDEN_LAYERS,
                activations,
                clustering_algorithm="kmeans",
            ...
            y_pred_test_shrinked = shrinked_model.predict(
                np.array(
                    X_test.values[example_index]).reshape(
                    (1, -1))).flatten()
            ...
            print(
                f"Both Shrunken model and the Original model produce exactly the same output for test "
                f"example at index {example_index}" if np.abs(
                    y_pred_test_shrinked[0] -
                    y_pred_test[0]) < 1e-6 else f"Shurnken model's output {y_pred_test[0]} is "
                f"different from Original model's output {y_pred_test_shrinked[0]}")
            print(
                f"ground_truth:{y_pred_test}, prediction:{y_pred_test_shrinked}")
            if np.argmax(y_pred_test) != np.argmax(y_pred_test_shrinked):
                misclassification += 1

            # make a vector from all weights of the shrunken network. This will
            ...
            # range(len(np.array(X_onehot_test))):
            test_index = example_index

            input = X_test.values[example_index]
            ...
            general_method_for_visualize_attack_and_supports_QBAF(
                ...
                'iris_local_graphs(shrunken_model)',
                0)
            ...
            general_clustered_visualize_attack_and_supports_QBAF(
                ...
                biases,
                Shrinkage_percentage,
                'iris_local_graphs(original_model)',
                0,
                clustering_labels)
            ...
            explanation, LIME_local_unfaithfulness = explainer.explain_instance(
                X_test.values[example_index], model.predict, num_features=len(feature_names), random_state=123, labels=[
                    0, 1, 2])
            ...
            for index, feature_name in enumerate(feature_names):
                ...
                per_example_feature_attribution_distance += np.abs(
                    scores[index][1] - scores_2[index][1])

            print(output)
            print(
                f"Example {test_index} feature attribution distance: {per_example_feature_attribution_distance}")
            ...
            original_activations = utility_functions.compute_activations_for_each_layer(
                model, X_test.values)
            shrunken_activations = utility_functions.compute_activations_for_each_layer(
                    ...
                    for cluster_label in range(
                        ...
                        if cluster_label in clustering_labels[i]:
                            structural_unfaithfulness += np.sum(np.abs(
                                np.mean(original_activation[example_index, clustering_labels[i] == cluster_label]) -
                                pruned_activation[
                                    example_index, cluster_label]))
                else:
                    structural_unfaithfulness += np.sum(np.abs(
                        pruned_activation[example_index, :] - original_activation[example_index, :]))
            number_of_nodes = sum(
                truncated_model_dimensions) + y_onehot_test.shape[1]
            ...
            # fidelity
            example_unfaithfulness = np.sum(
                np.multiply(
                    np.sum(
                        np.power(
                            shrinked_model.predict(
                                X_test.values) - model.predict(
                                X_test.values),
                            2),
                        axis=1),
        ...
        overal_unfaithfulness_list.append(
            overal_unfaithfulness / number_of_examples)
        overal_structural_unfaithfulness_list.append(
            overal_structural_unfaithfulness / number_of_examples)
        overal_LIME_local_unfaithfulness_list.append(
            overal_LIME_local_unfaithfulness / number_of_examples)
        print(
            f"Total feature attribution distance {feature_attribution_distance / number_of_examples}")
        print(
            f"Total LIME feature attribution similarity {1 - (feature_attribution_distance / number_of_examples)}")
        print(
            f"Overal Structural Unfaithfulness: {overal_structural_unfaithfulness / number_of_examples}")
        all_misclassifications.append(misclassification)
        feature_attribution_list.append(
            (feature_attribution_distance / number_of_examples))
...
print("Our average structural unfaithfulness: " +
      str(np.mean(overal_structural_unfaithfulness_list)))
