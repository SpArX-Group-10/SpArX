    ...
    for idx in range(len(non_zero_features)):
        if np.abs(feature_weights[idx]) >= weight_threshold:
            ...
            for i in range(word_frequency):
                text += str(
                    non_zero_features[idx]).replace(
                    ' ', '_').replace(
                    '-', '_') + " "
            if feature_weights[idx] >= weight_threshold:
                color_to_words['#00ff00'].append(
                    str(non_zero_features[idx]).replace(' ', '_').replace('-', '_'))
            elif feature_weights[idx] <= -weight_threshold:
                color_to_words['red'].append(
                    str(non_zero_features[idx]).replace(' ', '_').replace('-', '_'))
    if text != '':
        w = 500
        h = 500
        ...
        circle_mask = create_circular_mask(
            h, w, center=center, radius=radius) * 255
        ...
        # plt.show()
        plt.savefig(
            f"hidden_wordclouds/wc_{image_index}_{hidden_node_index}.png",
            bbox_inches='tight')
