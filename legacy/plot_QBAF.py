...
def xor_visualize_attack_and_supports_QBAF_Word_Clouds(input, output, feature_names, number_of_hidden_nodes,
                                                   edge_weight_threshold,
                                                   input_to_hidden_weights, hidden_to_output_weights, shrink_percentage,
                                                   hidden_activation, output_activation, path, fig_index,
                                                   hidden_bias='',
                                                   output_bias=''):
    # import module
    from graphviz import Digraph
    ...
    for i in range(number_of_hidden_nodes):
        if (max(np.array(input_to_hidden_weights)[i, :] * input) >= edge_weight_threshold or min(
                np.array(input_to_hidden_weights)[i, :] * input) <= - edge_weight_threshold) and (
                np.abs(np.array(np.sum(hidden_to_output_weights[i]))) >= edge_weight_threshold):
            # if i == 0:
            ...
            # else:
                dot.node('H' + str(i), label='', color='white',
                         width=str(1),#str(int(np.sqrt(np.abs(np.sum(hidden_to_output_weights[i]))))),
                         height=str(1),#str(int(np.sqrt(np.abs(np.sum(hidden_to_output_weights[i]))))),
                         fixedsize='true',
                         imagescale='true',
...
def visualize_attack_and_supports_QBAF_Word_Clouds(input, output, feature_names, number_of_hidden_nodes,
                                                   edge_weight_threshold,
                                                   input_to_hidden_weights, hidden_to_output_weights, shrink_percentage,
                                                   hidden_activation, output_activation, path, fig_index,
                                                   hidden_bias='',
