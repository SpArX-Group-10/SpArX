...
for Shrinkage_percentage in range(20, 90, 20):
    ...
    for j in range(0, number_of_tests):
        ...
        for example_index in range(
            ...
            # todo: change __data_inverse in lime_tablurar.py to data_inverse
            # to be able to use the function outside lime_tablurar.py
            data, inverse = explainer.data_inverse(
                X_test.values[example_index], 5000)
            scaled_data = (data - explainer.scaler.mean_) / \
            ...
            clustering_labels_with_no_zero = clustering_nodes(
                preserve_percentage, HIDDEN_LAYERS, activations, clustering_algorithm="kmeans")
            ...
            clustering_labels = clustering_the_zero_activations_out_to_a_new_cluster_for_an_example(
                activations, HIDDEN_LAYERS, 0, clustering_labels_with_no_zero)
            ...
            explanation, LIME_local_unfaithfulness = explainer.explain_instance(
                scaled_data[0], model.predict, num_features=len(feature_names), random_state=123, labels=[
                    0, 1, 2])
            ...
            explanation, _ = explainer.explain_instance(
                scaled_data[0], shrinked_model.predict, num_features=len(feature_names), random_state=123)
            ...
            example_unfaithfulness = np.sum(
                np.multiply(
                    np.sum(
                        np.power(
                            shrinked_model.predict(scaled_data) - model.predict(scaled_data),
                            2),
                        axis=1),
        ...
        print(
            f"Overal Unfaithfulness (ratio: {Shrinkage_percentage/100}): {overal_unfaithfulness / number_of_examples}")
        print(
...
plt.plot(
    Shrinkage_percentage_list,
    overal_unfaithfulness_list,
    label="Our Local Explanations")
plt.plot(
    Shrinkage_percentage_list,
    overal_LIME_local_unfaithfulness_list,
    'r',
    label="LIME local Explanations")
...
print("LIME average unfaithfulness: " +
      str(np.mean(overal_LIME_local_unfaithfulness_list)))
